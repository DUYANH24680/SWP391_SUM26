...
<head>
    ...
    <meta name="description" content="Khám phá tất cả danh mục sản phẩm tươi ngon tại Sena Shop – trái cây, rau củ, thực phẩm nhập khẩu và nhiều hơn nữa.">
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-avatar { width: 36px; height: 36px; border-radius: 50%; object-fit: cover; border: 2px solid var(--green); }
        .nav-icon-btn {
            background: none; border: none; width: 38px; height: 38px;
            border-radius: 50%; cursor: pointer; color: var(--gray-600);
        ...
        .hero-subtitle { font-size: 1rem; opacity: 0.85; position: relative; }
        .hero-badge {
        ...
        /* ===== LAYOUT ===== */
        .layout { max-width: 1300px; margin: 0 auto; padding: 2rem 1.5rem; }
        ...
        .cat-card.active .cat-name { color: var(--green-dark); font-weight: 700; }
        ...
        .cat-card.active .cat-img-wrap { transform: scale(1.08); }
        .cat-img-wrap img { width: 100%; height: 100%; object-fit: cover; }
        .cat-img-wrap i { font-size: 1.8rem; color: var(--green); }
        .cat-name { font-size: 0.85rem; font-weight: 600; line-height: 1.3; position: relative; }
        .cat-check {
        ...
        .filter-label { font-size: 0.875rem; font-weight: 600; color: var(--gray-600); white-space: nowrap; }
        .filter-selected {
            display: flex; align-items: center; gap: 0.5rem;
            background: var(--green-light);
        ...
        .product-count strong { color: var(--green-dark); font-weight: 700; }
        ...
        .prod-card:hover { box-shadow: var(--shadow-lg); transform: translateY(-4px); border-color: var(--green-mid); }

        .prod-img-wrap { position: relative; aspect-ratio: 1/1; overflow: hidden; background: var(--gray-50); }
        .prod-img-wrap img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.3s ease; }
        ...
        .prod-badge.in-stock { background: var(--green); }

        .prod-body { padding: 0.9rem; flex: 1; display: flex; flex-direction: column; gap: 0.35rem; }
        .prod-title {
            font-size: 0.9rem; font-weight: 700;
            line-height: 1.4; color: var(--gray-800);
            display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;
        }
        .prod-shop { font-size: 0.75rem; color: var(--gray-400); }
        .prod-price-row {
            display: flex; align-items: center; gap: 0.5rem;
            margin-top: auto; padding-top: 0.5rem;
        }
        .prod-sale-price { font-size: 1.05rem; font-weight: 800; color: var(--green-dark); }
        .prod-orig-price { font-size: 0.78rem; color: var(--gray-400); text-decoration: line-through; }
        .prod-unit { font-size: 0.72rem; color: var(--gray-400); margin-left: auto; }

        .prod-rating {
            display: flex; align-items: center; gap: 0.25rem;
            font-size: 0.75rem; color: #f59e0b;
        }
        .prod-rating span { color: var(--gray-500); }
        ...
        .empty-state .icon { font-size: 3.5rem; margin-bottom: 1rem; color: var(--gray-200); }
        ...
        .pick-prompt i { font-size: 3rem; margin-bottom: 0.75rem; display: block; color: var(--green-mid); }
        .pick-prompt p { font-size: 0.95rem; font-weight: 500; }
...
<nav class="topnav">
    ...
    <div class="nav-right">
        ...
        <% } else { %>
            <a href="<%= request.getContextPath() %>/login" class="nav-icon-btn" title="Đăng nhập">
...
<div class="hero">
    ...
    <p class="hero-subtitle">Chọn danh mục để xem các sản phẩm tươi ngon phù hợp với bạn</p>
</div>
...
<div class="layout">
    ...
    <% if (errorMsg != null) { %>
        <div style="background:#fee2e2;border:1px solid #fecaca;color:#991b1b;padding:0.9rem 1.2rem;border-radius:10px;margin-bottom:1.25rem;font-size:0.875rem;">
            ...
            <% for (Category cat : categories) { %>
                <%
                    ...
                    if (cat.getImage() != null && !cat.getImage().trim().isEmpty()) {
                        catImgUrl = imgUrl(cat.getImage(), request.getContextPath());
                    }
                %>
                <a href="<%= request.getContextPath() %>/danh-muc?categoryId=<%= cat.getId() %>"
                   class="cat-card <%= isSelected ? "active" : "" %>">
                    <div class="cat-check"><i class="fa-solid fa-check"></i></div>
                    <div class="cat-img-wrap">
                        <% if (catImgUrl != null) { %>
                            <img src="<%= catImgUrl %>" alt="<%= cat.getName() %>"
                                 onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
                            <i class="fa-solid fa-leaf" style="display:none;"></i>
                        <% } else { %>
    ...
    <% if (selectedCategoryId != null) { %>
        ...
        <div class="filter-bar">
            <span class="filter-label"><i class="fa-solid fa-filter"></i> Lọc theo:</span>
            <span class="filter-selected">
                <i class="fa-solid fa-tag"></i>
                <%= selectedCategoryName != null ? selectedCategoryName : "Danh mục #" + selectedCategoryId %>
            </span>
            <a href="<%= request.getContextPath() %>/danh-muc" class="filter-clear">
        ...
        <div class="products-header">
            <h2 class="section-title" style="margin-bottom:0;">
                ...
                Sản phẩm: <%= selectedCategoryName != null ? selectedCategoryName : "" %>
            </h2>
            <% if (products != null) { %>
                <div class="product-count">
                    <strong><%= products.size() %></strong> sản phẩm tìm thấy
                </div>
            <% } %>
        </div>
        ...
        <% } else { %>
            ...
            <% for (Product p : products) {
                String pImgUrl = imgUrl(p.getImage(), request.getContextPath());
                double discount = p.getDiscountPercent();
                boolean inStock = p.isInStock();
            %>
                <a href="<%= request.getContextPath() %>/product-info?id=<%= p.getId() %>" class="prod-card">
                    <div class="prod-img-wrap">
                        <% if (pImgUrl != null) { %>
                            <img src="<%= pImgUrl %>" alt="<%= p.getTitle() %>"
                        ...
                        <% } else { %>
                            <div class="prod-placeholder"><i class="fa-solid fa-image"></i></div>
                        <% } %>
                        <% if (discount > 0) { %>
                            <span class="prod-badge">-<%= (int) discount %>%</span>
                        <% } else if (inStock) { %>
                            <span class="prod-badge in-stock">Còn hàng</span>
                        <% } %>
                    </div>
                    <div class="prod-body">
                        <div class="prod-title"><%= p.getTitle() %></div>
                        <% if (p.getShopName() != null) { %>
                            <div class="prod-shop"><i class="fa-solid fa-store" style="font-size:0.7rem;"></i> <%= p.getShopName() %></div>
                        <% } %>
                        <% if (p.getAverageRating() > 0) { %>
                            <div class="prod-rating">
                                <i class="fa-solid fa-star"></i>
                                <%= String.format("%.1f", p.getAverageRating()) %>
                                <span>(<%= p.getSoldQuantity() %> đã bán)</span>
                            </div>
                        <% } %>
                        <div class="prod-price-row">
                            <span class="prod-sale-price"><%= formatPrice(p.getSalePrice()) %></span>
                            <% if (p.getOriginalPrice() > p.getSalePrice()) { %>
                                <span class="prod-orig-price"><%= formatPrice(p.getOriginalPrice()) %></span>
                            <% } %>
                            <span class="prod-unit">/<%= p.getUnit() != null ? p.getUnit() : "cái" %></span>
                        </div>
                    </div>
                    <div class="prod-footer">
                        <% if (inStock) { %>
                            <span class="btn-add-cart" onclick="event.preventDefault(); addToCart(<%= p.getId() %>)">
                                <i class="fa-solid fa-cart-plus"></i> Thêm vào giỏ
                            </span>
                        <% } else { %>
                            <span class="btn-add-cart" style="background:var(--gray-400);cursor:not-allowed;">
    ...
    <% } else { %>
        ...
        <div class="pick-prompt">
            ...
            <p>Hãy chọn một danh mục bên trên để xem sản phẩm!</p>
        </div>
    <% } %>
